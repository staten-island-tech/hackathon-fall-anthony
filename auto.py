...
RED = (247, 74, 76)
BLUE = (67, 171, 251)
MISS_RED = (241, 125, 126)
MISS_RED_SHADOW = (87, 1, 1)
PERFECT_BLUE = (204, 108, 231)
PERFECT_BLUE_SHADOW = (79, 2, 101)
GRAY = (200, 200, 200)
...
def draw_game() -> None:
    ...
    progress_bar = pygame.Rect((-8, -4), ((note_timer / note_instructions[-1]["time"]) * SCREEN_WIDTH + 8, 12))
    pygame.draw.rect(
        screen,
        GRAY,
        progress_bar,
    ...
    if feedback_timer > 0:
        ...
        feedback_text_shadow = small_font.render(
            f"{feedback}! x{combo}" if combo > 1 else f"{feedback}!",
            True,
            MISS_RED_SHADOW if feedback == "MISS" else PERFECT_BLUE_SHADOW if feedback == "PERFECT" else (100, 100, 100)
        )
        feedback_text = small_font.render(
            f"{feedback}! x{combo}" if combo > 1 else f"{feedback}!",
            True,
            MISS_RED if feedback == "MISS" else PERFECT_BLUE if feedback == "PERFECT" else GRAY
        )
        feedback_text_shadow.set_alpha(feedback_alpha)
        feedback_text.set_alpha(feedback_alpha)
        feedback_text_shadow = pygame.transform.scale(feedback_text_shadow, (int(feedback_text_shadow.get_width() * feedback_scale), int(feedback_text_shadow.get_height() * feedback_scale)))
        ...
        screen.blit(feedback_text_shadow, (feedback_x - feedback_text_shadow.get_width() // 2 + 3, feedback_y - feedback_text_shadow.get_height() // 2 + 3))
